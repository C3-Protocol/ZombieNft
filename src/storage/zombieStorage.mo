...
shared(msg) actor class ZombieStorage(_owner: Principal) {
    ...
    private stable var userFavoriteEntries: [(Principal, [ZombieStoreCID])] = [];
    private var userFavorite = HashMap.HashMap<Principal, [ZombieStoreCID]>(1, Principal.equal, Principal.hash);
    ...
    system func postupgrade() {
        userFavorite := HashMap.fromIter<Principal, [ZombieStoreCID]>(userFavoriteEntries.vals(), 1, Principal.equal, Principal.hash);
    ...
    public shared(msg) func setFavorite(user: Principal, info: ZombieStoreCID) : async () {
        assert( msg.caller == multiNft_canister_id_);
        
        var ret: Bool = false; 
        switch(userFavorite.get(user)){
            case (?arr){
                if(Option.isNull(Array.find<ZombieStoreCID>(arr, func (x : ZombieStoreCID): Bool { x.index == info.index })))
                {
                    var arrNew : [ZombieStoreCID] = Array.append(arr, [info]);
    ...
    public shared(msg) func cancelFavorite(user: Principal, info: ZombieStoreCID) : async () {
        assert( msg.caller == multiNft_canister_id_);
        
        var ret: Bool = false; 
        switch(userFavorite.get(user)){
            case (?arr){
                var arrNew : [ZombieStoreCID] = Array.filter<ZombieStoreCID>(arr, func (x : ZombieStoreCID): Bool { x.index != info.index } );
                userFavorite.put(user, arrNew);
                ret := true;
            };
            case _ {};
    ...
    public query func getFavorite(user: Principal): async [ZombieStoreCID] {
        var ret: [ZombieStoreCID] = [];
        switch(userFavorite.get(user)){
            case (?arr){
                ret := arr;
            };
    ...
    public query func isFavorite(user:Principal, info: ZombieStoreCID): async Bool {
        var ret: Bool = false;
        switch(userFavorite.get(user)){
            case (?arr){
                for (v in arr.vals()) {
